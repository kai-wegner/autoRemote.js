...
var getServerConfig = function(serverConfig) {
    ...
    if (this._serverConfig == null && serverConfig == null) {
        this._serverConfig = {
            interfaces: {
                public: {},
                local: {}
        ...
        _.extend(this._serverConfig, serverConf);

        resolveConfig(promise);
    }
    // merge current _serverConfig and supplied serverConfig
    else if (this._serverConfig != null && serverConfig != null) {
        _.extend(serverConfig, this._serverConfig);
    ...
    // return the supplied serverConfig since there is nothing we can merge it with
        ...
        promise.resolve(serverConfig);
    // otherwise
    else promise.resolve(this._serverConfig);

    return promise.promise;
}
exports.getServerConfig = getServerConfig;

var resolveConfig = function(callback) {
    if(this._serverConfig != null && this._serverConfig.interfaces != null) {
        var localIP_promise = getIP(this._serverConfig.interfaces.local);
        var publicIP_promise = getIP(this._serverConfig.interfaces.public);

        var thiz = this;
        when.all([localIP_promise, publicIP_promise])
            .then(function(ips) {
                thiz._serverConfig.interfaces.local.ip = ips[0];
                thiz._serverConfig.interfaces.public.ip = ips[1];

                if(callback.resolve != null)
                    callback.resolve(thiz._serverConfig);
                else if(callback != null)
                    callback(thiz._serverConfig);
            });
    }
    else throw "No interfaces found!";
}
...
**/
var registerServerToDevice = function(serverConfig, device, callback) {
    this.getServerConfig(serverConfig).
...
exports.registerServerToDevice = registerServerToDevice;
