...
var doPostString = function(ipaddress, port, paramsString, contentType, callback, callbackError) {
    ...
    var normalLength = post_data.length;
    var utf8Length = lengthInUtf8Bytes(post_data);
    //console.log("normal length: " + normalLength + "; " + "utf8: " + utf8Length);
    var post_options = {
        host: ipaddress,
        method: 'POST',
        path: "/",
...
var Communication = function() {
    ...
    this.send = function(device, callbackResponse) {
        ...
        var gcmFallback = function(responseObj) {
            if (me.key != null) {
                ...
                doPost(url, params, function(responseFromServer) {
                    var response = new ResponseNoAction();
                    var title = null;
                    var description = null;
                    console.log("Send response: " + responseFromServer);
                    if (responseFromServer == 'OK') {
                        ...
                        description = responseFromServer;
                        response.responseError = responseFromServer;
                    }
                ...
                console.log("Couldn't send through local ip and device doesn't have a GCM key.");
                notify('Error', 'Can\'t send ' + me.getCommunicationType());
                callbackResponse(null);
            }
        };
        ...
        doPostString(device.localip, device.port, params, "application/json", function(response) {
            try {
                ...
                callbackResponse(responseObj);
            } catch(exception) {
                gcmFallback(response)
            }
        }, gcmFallback);
...
exports.ResponseGetRegistration = ResponseGetRegistration;
